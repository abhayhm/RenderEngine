...
void triangles_from_polygon(polygon_t* polygon, triangle_t triangles[], int* num_triangles) {
	for (int i = 0; i < polygon->num_vertices - 2; i++) {
		...
		triangles[i].texcoords[0] = polygon->texcoords[index_0];
		triangles[i].texcoords[1] = polygon->texcoords[index_1];
		triangles[i].texcoords[2] = polygon->texcoords[index_2];
	}
...
void clip_polygon_against_plane(polygon_t* polygon, int plane) {
	...
	// Static array which will be part of final polygon returned via parameter.
	vec3_t inside_vertices[MAX_NUM_POLY_VERTICES];
	tex2_t inside_texcoords[MAX_NUM_POLY_VERTICES];
	int num_inside_vertices = 0;

	// Starting current vertex with first polygon vertex
	...
	// Loop all the polygon vertices
	while (current_vertex != &polygon->vertices[polygon->num_vertices]) {
		current_dot = vec3_dot(vec3_sub(*current_vertex, plane_point), plane_normal);

		// If we change from inside to outside or outside to inside
		if (current_dot * previous_dot < 0) {

			// Finding interpolation factor
			float t = previous_dot / (previous_dot - current_dot);

			// Calculate interpolation point I = Q1 + t(Q2 - Q1)
		...
		if (current_dot > 0) {
			// Insert current vertex to the list "Inside vertices"
	...
	// Copy list of vertices into destination polygon
